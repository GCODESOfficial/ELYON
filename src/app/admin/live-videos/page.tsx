...
export default function AdminLiveVideosPage() {
  const [liveVideos, setLiveVideos] = useState<{ id: string; images: string[] }[]>([])
  ...
  const fetchLiveVideos = async () => {
    setLoading(true)
    const { data, error } = await supabase.from("live_videos").select("id, images")
    if (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-[#0D1B2A]">Live Video Images</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-[#CFA83C]">No live video images found.</div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {liveVideos.map((video) => (
            <div key={video.id} className="bg-white rounded-lg shadow p-4 relative">
              <button
              ...
              <div className="grid grid-cols-1 gap-2">
                {video.images.map((img, idx) => (
                  <div key={idx} className="relative aspect-square overflow-hidden rounded-lg">
                    <Image src={img} alt={`Live Video ${idx + 1}`} fill className="object-cover" />
                  </div>
                ))}
              </div>
            </div>
